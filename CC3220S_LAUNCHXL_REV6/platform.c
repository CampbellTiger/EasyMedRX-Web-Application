...
#define APPLICATION_NAME1                     "HTTPS GET"
#define APPLICATION_NAME2                     "HTTPS POST"
...
extern void* httpsPost(void* pvParameters);
...
void SimpleLinkNetAppEventHandler(SlNetAppEvent_t *pNetAppEvent)
{
    ...
    case SL_NETAPP_EVENT_IPV6_ACQUIRED:
        ...
        if(mode != ROLE_AP)
        {
            ...
            Display_printf(display, 0, 0, "Wifi succeeded, spawning HTTPS REST\n\r");

            pthread_attr_init(&pAttrs);
            priParam.sched_priority = 1;
            status = pthread_attr_setschedparam(&pAttrs, &priParam);
            status |= pthread_attr_setstacksize(&pAttrs, TASK_STACK_SIZE);
            status = pthread_create(&httpsThread, &pAttrs, httpsPost, NULL);                            /*http vs https*/
...
int Connect(void)
{
    ...
    Display_printf(display, 0, 0, "Connect successful, ret = \n", ret);
    return 0;
}
